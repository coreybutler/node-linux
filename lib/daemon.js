...
var daemon = function(config) {
  ...
  // Generate wrapper code arguments
	var args = [
// sellis - escape the double quotes here because we will put them inside an su -c "expandedcommand"
// only used by systemv script to run service as specific user
    '-f','\\"'+this.script.trim()+'\\"',
    '-l','\\"'+this.outlog.trim()+'\\"',
    '-e','\\"'+this.errlog.trim()+'\\"',
    '-t','\\"'+this.label.trim()+'\\"',
